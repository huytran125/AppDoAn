...
const Infovehicle = props => {
          ...
          position => {
            fetchAddress({
              latitude:
                ...
                (Math.random() - 0.5) * LATITUDE_DELTA,
              longitude:
                position.coords.longitude +
                (Math.random() - 0.5) * LATITUDE_DELTA,
            });
            fetchDataDevice(position);
  ...
  const fetchDataDevice = async position => {
    const response = await fetch(
    ...
    fetchDistance(
      ...
      {
        latitude:
          position.coords.latitude + (Math.random() - 0.5) * LATITUDE_DELTA,
        longitude:
          position.coords.longitude + (Math.random() - 0.5) * LATITUDE_DELTA,
      },
    );
  ...
  return (
    <View style={styles.screen}>
      <View
        ...
        <Text style={{fontWeight: 'bold'}}>Device: {yourDevice.name}</Text>
        <Text style={{fontWeight: 'bold'}}>Username: {curUser.name}</Text>
      </View>
      <View style={{width: '100%', height: '30%'}}>
        ...
        <View style={{paddingLeft: 20, marginTop: 20, height: '60%'}}>
          <Text>
            ...
            {yourCurentLocation.latitude} và {yourCurentLocation.longitude}
          </Text>
        </View>
      </View>
      <View
        ...
        <View
          ...
          <View
            ...
            <Text style={{fontSize: 16}}> {yourDevice.name}</Text>
          </View>
          <View
            style={{
              borderWidth: 1,
        ...
        <View
          ...
          <Text>Theo đường chim bay: {currentDistance.toFixed(4)} km</Text>
